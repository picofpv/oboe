...
public class MyAudioEngine {
    ...
    private static final String TAG = MyAudioEngine.class.getName();
    ...
    static boolean create(Context context) {
        if (mEngineHandle == 0){
            ...
            mEngineHandle = native_createEngine(); // 这里会得到一个由 Oboe 提供的 long
        }
        return (mEngineHandle != 0);    // 只要不为0，就表示成功创建了
    }
    ...
    private static void setDefaultStreamValues(Context context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1){
            ...
            Log.i(TAG,defaultSampleRate+","+defaultFramesPerBurst);
    ...
    // Native methods
    private static native long native_createEngine(); // 创建时可以全默认
